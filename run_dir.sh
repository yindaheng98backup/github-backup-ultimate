...
GE_USER=$3
GE_TOKEN=$4
REPOS_URL='https://api.github.com/user/repos?access_token='$GH_TOKEN'&type=owner'
GE_PREFIX='https://'$GE_USER':'$GE_TOKEN'@gitee.com/'$GE_USER
curl -s $REPOS_URL | jq -c '.[].clone_url' | while read URL; do
    ...
    cd ./main
    git push --all -f -u $GE_PREFIX'/'$REPO_NAME #直接强推到gitee
    #TODO: 仓库不存在时创建
